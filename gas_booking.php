...
<?php
$sql="select registration.name,registration.username,registration.mob,registration.consumer,registration.adress from registration inner join connection on 
...
<div class="form-group"><label for="">Mobile</label><input type="text" name="mob" value="<?= $r['mob']?>" class="form-control"></div>
<div class="form-group"><label for="">Delivery Address</label><textarea name="address" id="" cols="30" rows="2" class="form-control"><?= $r['adress']?></textarea></div>
<div class="form-group text-center"><button type="submit" class="btn btn-primary" name="id" value="<?= $id ?>">SUBMIT</button></div>
</form>
</div>
...
extract($_POST);
print_r($_POST);
echo $bookingdate=date('Y-m-d');
$sql="INSERT INTO `booking`(`regid`, `name`, `username`, `consumer`, `mob`, `address`, `bookingdate`, `quantity`, `amount`) 
VALUES ($id,'$name','$username','$consumer','$mob','$address','$bookingdate', '1', 'amount')";
if(mysqli_query($con,$sql))
{
  ...
  $_SESSION['regid']=$id;
    ...
    echo("<script>location.href = 'gas_booking.php';</script>");
}
}
?>
<br>
